...
var nexaRules = {
    ...
    waive_open_access: {
        ...
        compliant_values: function (v) {
            return ((v === "no") ? 0.75 : (v === "yes") ? 0.25 : false);
        },
        explanation: {
            0.75: "is 'no'",
            0.25: "is 'yes'",
    ...
    maximal_embargo_waivable: {
        field_id: 17,
        compliant_values: function (v) {
            return ((["no", "not_specified"].indexOf(v) >= 0) ? true :
                    (v === "yes") ? 0.25 : false);
        },
        guidelines: ["3.14"],
        gmga: ["29.2.2.b"],
...
var evaluateRule = exports.evaluateRule = function (
            rule, record, key, value, callback) {
    var result = {
    ...
    if (typeof rule.compliant_values === "string") {
        result.score = (value === rule.compliant_values);
        result.reason = ((!result.score) ? "not " : "") + "equal to " +
                "'" + rule.compliant_values + "'";

    } else if (rule.compliant_values instanceof Array) {
        result.score = (rule.compliant_values.indexOf(value) >= 0);
        result.reason = ((!result.score) ? "not " : "") + "one of: " +
                rule.compliant_values.join(", ");

    } else if (rule.compliant_values instanceof Function) {
        result.score = rule.compliant_values(value, record, key);
        result.reason = rule.explanation[result.score];

    } else {
        throw new Error("Programmer error");
    }

    result.value = (rule.normalize && rule.normalize(value)) || value;
    result.caveats = (rule.caveats && rule.caveats[result.score]);
    result.is_compliant = (result.score >= 0.5);
...
var apply = exports.apply = function (record, callback) {
    var newRecord = [];
    Object.keys(nexaRules).forEach(function (key) {
        newRecord.push(
            evaluateRule(nexaRules[key], record, key, record[key], callback)
        );
    });
    return newRecord;
};
