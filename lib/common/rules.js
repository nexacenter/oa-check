...
var evaluateRule = function (rule, record, key, value) {
    ...
    if (typeof rule.compliant_values === "string") {
        result.is_compliant = (value === rule.compliant_values);
        result.reason = ((!result.is_compliant) ? "not " : "") + "equal to "
                + "'" + rule.compliant_values + "'";

    } else if (rule.compliant_values instanceof Array) {
        result.is_compliant = (rule.compliant_values.indexOf(value) >= 0);
        result.reason = ((!result.is_compliant) ? "not " : "") + "one of: "
                + rule.compliant_values.join(", ");

    } else if (rule.compliant_values instanceof Function) {
        result.is_compliant = rule.compliant_values(value, record, key);
        result.reason = rule.explanation[result.is_compliant];
    ...
    result.fuzzyLabel = (rule.caveats && rule.caveats[result.is_compliant]);

    // XXX
    result.is_compliant_float = result.is_compliant;
    result.is_compliant = (result.is_compliant >= 0.5);
    result.fuzzy_label = result.fuzzyLabel;
...
var apply = exports.apply = function (record) {
    ...
    Object.keys(nexaRules).forEach(function (key) {
        newRecord.push(
            evaluateRule(nexaRules[key], record, key, record[key])
        );
    });
    return newRecord;
};
